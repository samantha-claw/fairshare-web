    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="rounded-2xl bg-white p-8 text-center shadow-lg">
          <h2 className="text-xl font-semibold text-red-600">{g.error || "Group not found"}</h2>
          <button onClick={g.goBack} className="mt-4 text-blue-600 hover:underline">Go Back</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="border-b border-gray-200 bg-white px-6 py-6">
        <div className="mx-auto flex max-w-6xl items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{g.group.name}</h1>
            <p className="mt-1 text-sm text-gray-500">
              {g.members.length} member{g.members.length !== 1 && "s"} · {currency}
            </p>
          </div>
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="p-6">
                ...
                {g.activeTab === "activity" && (
                  <ActivityTab allActivities={g.allActivities} currency={currency} />
                )}
              </div>
            </section>
          </div>
