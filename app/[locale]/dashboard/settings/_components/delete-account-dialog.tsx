...
export function DeleteAccountDialog({ username }: DeleteAccountDialogProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        t("deleteAccount.errors.success")
      );
      ...
      toast.error(t("deleteAccount.errors.unexpected"));
      setIsDeleting(false);
    }
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-2.5 rounded-xl border border-red-100 bg-red-50/50 p-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm text-text-primary">
                  {t("deleteAccount.consequences.removedFromGroups")}
                </p>
              </div>
              <div className="flex items-start gap-3">
                <Bell className="mt-0.5 h-4 w-4 flex-shrink-0 text-red-500" />
                <p className="text-sm text-text-primary">
                  {t("deleteAccount.consequences.deletedFriendships")}
                </p>
              </div>
              <div className="flex items-start gap-3">
                <Database className="mt-0.5 h-4 w-4 flex-shrink-0 text-red-500" />
                <p className="text-sm text-text-primary">
                  {t("deleteAccount.consequences.preservedRecords")}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1">
              ...
              >
                {t("deleteAccount.typeUsername", { username })}
              </label>
              <input
