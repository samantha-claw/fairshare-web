        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          ...
          <div className="xl:col-span-2 flex flex-col gap-5">
            ...
            <div className="bg-surface rounded-2xl p-6 border border-border flex-1 flex flex-col relative min-h-[350px]">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-text-primary">{t('dashboard.financialView')}</h2>
                <div className="relative">
                  <select
                    value={selectedRange}
                    onChange={(e) => setSelectedRange(e.target.value)}
                    className="appearance-none bg-surface-2 border-none rounded-xl ps-4 pe-10 py-2 text-sm font-medium text-text-secondary focus:ring-0 cursor-pointer"
                  >
              ...
              <div className="mt-16 flex-1 overflow-x-auto pb-2" style={{ WebkitOverflowScrolling: "touch" }}>
                <div className="flex min-w-max items-end justify-end gap-1 sm:gap-2 md:gap-3 pb-6 pe-4">
                      ...
                      >
                        {isActive && (
                          <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-surface text-text-primary px-3 py-1 rounded-lg text-xs font-bold shadow-md whitespace-nowrap">
                            {group.net_balance > 0 ? "+" : ""}{group.net_balance.toLocaleString(locale === "ar" ? "ar-SA" : "en-US", { maximumFractionDigits: 0 })}
              ...
              {/* Total amount */}
              <div className="absolute bottom-6 start-6 flex flex-col">
                <div className="text-3xl font-bold leading-none tracking-tight flex items-baseline">
                  <span className="text-text-secondary text-lg me-1">$</span>
                  <span
                    className={
                      d.totalNet > 0
                        ? "text-positive"
                  ...
                  >
                    {d.totalNet.toLocaleString(locale === "ar" ? "ar-SA" : "en-US")}
                  </span>
                </div>
                <span className="text-xs text-text-tertiary mt-1">{t('dashboard.netBalance')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5" style={{ animationDelay: '0.05s' }}>
            ...
            <div className="bg-surface rounded-2xl p-6 border border-border flex flex-col">
              <h2 className="text-lg font-bold mb-6 text-text-primary">{t('dashboard.spendingBreakdown')}</h2>
              ...
              <div className="relative w-48 h-48 mx-auto my-4">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-xs text-text-secondary mt-1">{t('dashboard.ofTotal')}</span>
                </div>
              </div>
            ...
            <div className="bg-surface rounded-2xl p-6 border border-border flex-1 flex flex-col">
              <h2 className="text-lg font-bold mb-4 text-text-primary">{t('dashboard.popularGroups')}</h2>
