      ...
      .map(([period, total]) => ({
        ...
        label:
          timeRange === "week" || timeRange === "2weeks"
            ? new Date(period + "T00:00:00").toLocaleDateString(dateLocale, {
                month: "short",
                day: "numeric",
              })
            : new Date(period + "-01").toLocaleDateString(dateLocale, {
                month: "short",
                year: "2-digit",
              }),
      }));
  ...
  const categoryTrendData = useMemo(() => {
    if (timeRange === "week" || timeRange === "2weeks") {
      ...
      return [...Array.from(dayMap.entries())]
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([day, catData]) => ({
          label: new Date(day + "T00:00:00").toLocaleDateString(dateLocale, {
            month: "short",
            day: "numeric",
          }),
          ...Object.fromEntries(
      ...
      return [...Array.from(monthMap.entries())]
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([month, catData]) => ({
          label: new Date(month + "-01").toLocaleDateString(dateLocale, {
            month: "short",
            year: "2-digit",
          }),
          ...Object.fromEntries(
