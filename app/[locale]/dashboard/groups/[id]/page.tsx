...
export default function GroupDetailsPage() {
      ...
      <div className="flex min-h-screen items-center justify-center bg-surface-2">
        <div className="rounded-2xl bg-surface p-8 text-center shadow-lg">
          <h2 className="text-xl font-semibold text-negative">
            {data.error || t('groups.groupNotFound')}
          </h2>
          <button
            onClick={data.goBack}
  ...
  const fabOptions = [
    {
      label: t('groupTabs.expenses'),
      onClick: expenseCtl.openAddExpenseModal,
      Icon: <Receipt className="h-4 w-4" />,
    },
    {
      label: t('groupTabs.settle'),
  ...
  return (
    <div className="min-h-screen bg-surface-2 pb-24">
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-text-secondary">
              {data.members.length} {data.members.length !== 1 ? t('common.members') : t('common.member')} ·{" "}
              {currency}
            </p>
          </div>
      ...
      <div className="mx-auto max-w-4xl px-4">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="mt-4">
            <TabsTrigger value="overview">{t('groupTabs.overview')}</TabsTrigger>
            <TabsTrigger value="expenses">
              {t('groupTabs.expenses')}
            </TabsTrigger>
            <TabsTrigger
              value="settle"
            ...
            <TabsTrigger value="activity">{t('groupTabs.activity')}</TabsTrigger>
            <TabsTrigger value="analysis">{t('groupTabs.analysis')}</TabsTrigger>
            <TabsTrigger value="members">{t('groupTabs.members')}</TabsTrigger>
          </TabsList>
      ...
      <QRShareModal
        ...
        subtitle={`${data.members.length} ${data.members.length !== 1 ? t('common.members') : t('common.member')} · ${currency}`}
        type="group"
        isOwner={data.isOwner}
