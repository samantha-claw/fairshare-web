...
export default function RegisterPage() {
  ...
  const getPasswordStrength = (
    ...
    if (pw.length < 6) return { label: t('auth.register.passwordTooShort'), color: "bg-negative", width: "20%" };
    let score = 0;
    ...
    if (score <= 1) return { label: t('auth.register.passwordWeak'), color: "bg-orange-500", width: "40%" };
    if (score <= 2) return { label: t('auth.register.passwordFair'), color: "bg-amber-500", width: "60%" };
    if (score <= 3) return { label: t('auth.register.passwordStrong'), color: "bg-positive", width: "80%" };
    return { label: t('auth.register.passwordExcellent'), color: "bg-cyan-500", width: "100%" };
  };
  ...
  const usernameErrorMessage = (() => {
    ...
    if (usernameFormatValid === false) {
      return t('auth.register.usernameFormatHint');
    }
    if (usernameStatus === "taken") return t('auth.register.usernameTaken');
    if (usernameStatus === "error") {
      return t('auth.register.usernameError');
    }
    return "";
  })();
    ...
    return (
      ...
      >
        <div className="rounded-3xl border border-border bg-surface p-8 text-center shadow-lg">
          ...
          <h2 className="text-2xl font-bold text-text-primary">{t('auth.register.checkEmailTitle')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="group">
            ...
            <div className="relative">
              <User className="pointer-events-none absolute start-4 top-1/2 h-5 w-5 -translate-y-1/2 text-text-tertiary transition-colors group-focus-within:text-text-primary" />
              <input
                id="fullName"
                type="text"
                value={fullName}
                ...
                required
                className="w-full rounded-xl border border-border bg-surface py-3.5 ps-12 pe-4 text-sm text-text-primary outline-none transition-all placeholder:text-text-tertiary focus:border-border-2 focus:ring-2 focus:ring-border/30"
              />
            </div>
          ...
          <div className="group">
            ...
            <div className="relative">
              <AtSign className="pointer-events-none absolute start-4 top-1/2 h-5 w-5 -translate-y-1/2 text-text-tertiary transition-colors group-focus-within:text-text-primary" />
              <input
                id="username"
                type="text"
                value={username}
                ...
                required
                className={`w-full rounded-xl border bg-surface py-3.5 ps-12 pe-12 text-sm text-text-primary outline-none transition-all placeholder:text-text-tertiary focus:ring-2 ${getUsernameBorderClass()}`}
              />
              <div className="absolute end-4 top-1/2 -translate-y-1/2">{renderUsernameIcon()}</div>
            </div>
            ...
            {(usernameFormatValid === null || usernameFormatValid === false) && (
              <div className="mt-2 flex items-center gap-1.5">
                ...
                <p className={`text-[11px] ${usernameFormatValid === false ? "text-negative" : "text-text-tertiary"}`}>
                  {t('auth.register.usernameFormatHint')}
                </p>
              </div>
            )}
            {usernameFormatValid && usernameStatus === "checking" && (
              <div className="mt-2 flex items-center gap-1.5">
                ...
                <p className="text-[11px] text-amber-600">{t('auth.register.usernameChecking')}</p>
              </div>
            )}
            {usernameFormatValid && usernameStatus === "available" && (
              <div className="mt-2 flex items-center gap-1.5">
                <CheckCircle2 className="h-3 w-3 text-positive" />
                <p className="text-[11px] text-positive">{t('auth.register.usernameAvailable')}</p>
              </div>
            )}
            {usernameFormatValid && usernameStatus === "taken" && (
              <div className="mt-2 space-y-2.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[11px] text-negative">{t('auth.register.usernameTaken')}</p>
                </div>
                {usernameSuggestions.length > 0 && (
                  <div className="rounded-xl border border-border bg-surface-2 p-3">
                    <div className="mb-2 flex items-center gap-1.5">
                      <Sparkles className="h-3 w-3 text-text-primary" />
                      <p className="text-[11px] font-medium text-text-secondary">{t('auth.register.availableAlternatives')}</p>
                    </div>
            ...
            {usernameFormatValid && usernameStatus === "error" && (
              <div className="mt-2 flex items-center gap-1.5">
                ...
                <p className="text-[11px] text-amber-600">{t('auth.register.usernameError')}</p>
              </div>
            )}
          ...
          <div className="group">
            ...
            <div className="relative">
              <Mail className="pointer-events-none absolute start-4 top-1/2 h-5 w-5 -translate-y-1/2 text-text-tertiary transition-colors group-focus-within:text-text-primary" />
              <input
                id="email"
                type="email"
                value={email}
          ...
          <div className="group">
            ...
            <div className="relative">
              <Lock className="pointer-events-none absolute start-4 top-1/2 h-5 w-5 -translate-y-1/2 text-text-tertiary transition-colors group-focus-within:text-text-primary" />
              <input
                id="password"
                ...
                required
                minLength={6}
                className="w-full rounded-xl border border-border bg-surface py-3.5 ps-12 pe-12 text-sm text-text-primary outline-none transition-all placeholder:text-text-tertiary focus:border-border-2 focus:ring-2 focus:ring-border/30"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute end-4 top-1/2 -translate-y-1/2 text-text-tertiary transition-colors hover:text-text-primary"
            ...
            {password.length > 0 && (
              <div className="mt-3">
                ...
                <p id="password-strength" className="mt-1.5 text-end text-[11px] text-text-tertiary">
          ...
          <div className="group">
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                minLength={6}
                className={`w-full rounded-xl border bg-surface py-3.5 ps-12 pe-20 text-sm text-text-primary outline-none transition-all placeholder:text-text-tertiary focus:ring-2 ${
